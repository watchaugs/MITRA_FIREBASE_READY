...
// ── ADD this new function right after initCompliancePage() ends: ──────
async function loadOfficersFromServer() {
  try {
    ...
    if (!res.ok) return; // Silently skip if not admin or API error
 
    const data = await res.json();
 
    // Populate Grievance Officer fields
    if (data.grievance_officer) {
      const go = data.grievance_officer;
      const nameEl  = document.getElementById('go-name');
      const emailEl = document.getElementById('go-email');
      const phoneEl = document.getElementById('go-phone');
      if (nameEl && go.name)   nameEl.value  = go.name;
      if (emailEl && go.email) emailEl.value = go.email;
      if (phoneEl && go.phone) phoneEl.value = go.phone;
    }
 
    // Populate DPO fields
    if (data.dpo) {
      const dpo = data.dpo;
      const dpoNameEl  = document.getElementById('dpo-name');
      const dpoEmailEl = document.getElementById('dpo-email');
      const dpoPhoneEl = document.getElementById('dpo-phone');
      if (dpoNameEl && dpo.name)   dpoNameEl.value  = dpo.name;
      if (dpoEmailEl && dpo.email) dpoEmailEl.value = dpo.email;
      if (dpoPhoneEl && dpo.phone) dpoPhoneEl.value = dpo.phone;
 
      // If DPO is already saved, show the summary card instead of the input form
      ...
      if (summaryName) summaryName.innerText = dpo.name;
      if (dpo.name && cmdCenter && summarySec) {
        cmdCenter.style.display  = 'none';
        summarySec.style.display = 'block';
      }
    }
  } catch (e) {
    // Silent fail — page still works, fields just stay blank
    console.warn('[compliance] Could not load saved officer data:', e.message);
  }
}
...
async function saveGrievanceOfficer() {
  ...
  const email = (document.getElementById('go-email') || {}).value || '';
  const phone = (document.getElementById('go-phone') || {}).value || '';
 
  if (!name || !email) {
    showToast('⚠️ Please enter both Name and Email for the Grievance Officer.');
    return;
  }
 
  // Send EXACTLY the format the backend expects
  const payload = {
    grievance_officer: { name, email, phone }
  };
  ...
  try {
    ...
    if (response.status === 401) {
      showToast('⚠️ Unauthorized. Please log in again.');
      return;
    }
    ...
    showToast('✅ Grievance Officer details saved successfully');
  } catch (e) {
    console.error('[saveGrievanceOfficer]', e);
    showToast('❌ Network error — could not save officer details.');
  }
}
